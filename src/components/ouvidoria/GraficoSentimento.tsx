"use client";

import { Cell, Pie, PieChart, Tooltip } from "recharts";
import { SENTIMENTO_MANIFESTACAO } from "@/lib/ouvidoria/mock";

export function GraficoSentimento() {
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-400">Sentimento da manifestação</p>
      <div className="flex items-center gap-4">
        <div className="shrink-0">
          <PieChart width={160} height={160}>
            <Pie
              data={SENTIMENTO_MANIFESTACAO}
              dataKey="valor"
              nameKey="nome"
              cx={80}
              cy={80}
              innerRadius={0}
              outerRadius={72}
              stroke="#fcfcfb"
              strokeWidth={2}
              isAnimationActive={false}
            >
              {SENTIMENTO_MANIFESTACAO.map((entrada) => (
                <Cell key={entrada.nome} fill={entrada.cor} />
              ))}
            </Pie>
            <Tooltip formatter={(value) => `${value}%`} />
          </PieChart>
        </div>
        <ul className="flex flex-col gap-2 text-sm">
          {SENTIMENTO_MANIFESTACAO.map((entrada) => (
            <li key={entrada.nome} className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: entrada.cor }} />
              <span className="text-slate-600">{entrada.nome}</span>
              <span className="font-semibold tabular-nums text-slate-800">{entrada.valor}%</span>
            </li>
          ))}
        </ul>
      </div>
    </div>
  );
}

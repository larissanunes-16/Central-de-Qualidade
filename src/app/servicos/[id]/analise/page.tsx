...
export default function AnalisePage({ params }: { params: { id: string } }) {
    ...
    fetch(`/api/servicos/${params.id}`)
      ...
      .then((data) => {
        ...
        if (!cicloIdRef.current) {
          setErro("Nenhum ciclo de análise encontrado para este serviço.");
        }
      });
  }, [params.id]);
  ...
  async function tentarNovamente() {
    setErro(null);
    const res = await fetch(`/api/servicos/${params.id}/iniciar-analise`, { method: "POST" });
  ...
  async function irParaMaterializacao() {
    if (!ciclo?.relatorio) return;
    setAvancando(true);
    const res = await fetch(`/api/relatorios/${ciclo.relatorio.id}/aceitar`, { method: "POST" });
    const data = await res.json();
    setAvancando(false);
    if (res.ok) router.push(`/servicos/${data.servicoId}/materializacao`);
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Relatório de Achados</h1>
        ...
        {relatorio && (
          <div className="flex gap-2">
            ...
            <button
              onClick={irParaMaterializacao}
              disabled={avancando}
              className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50"
            >
              {avancando ? "Avançando..." : "Ir para materialização"}
            </button>
          </div>
        )}
      </div>
      ...
      {!erro && processando && (
        <div className="rounded-xl border border-slate-200 bg-white p-8 text-center">
          <p className="mb-3 text-sm font-medium text-slate-600">Processando documentos com IA...</p>
          <div className="mx-auto h-2 w-full max-w-md overflow-hidden rounded-full bg-slate-100">
            <div className="h-full rounded-full bg-brand-500 transition-all" style={{ width: `${percentualEstimado}%` }} />
          </div>
      ...
      {!erro && !processando && relatorio && (
        <RelatorioViewer relatorio={relatorio} editavel onSalvar={salvarEdicao} />
      )}
    </div>
  );
}
